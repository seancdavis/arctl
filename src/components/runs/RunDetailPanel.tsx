...
export function RunDetailPanel({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-full sm:max-w-lg bg-[var(--surface-2)] z-50 border-l border-[var(--border)] shadow-2xl flex flex-col transition-transform duration-200 ${isVisible ? "translate-x-0" : "translate-x-full"}`}
      >
        {/* Header */}
                ...
                {isLoadingSessions ? (
                  ...
                  <div className="space-y-2">
                    {sessions.map((session, i) => (
                      <div
                        key={session.id}
                        className="bg-[var(--surface-3)] rounded-lg px-3 py-2.5 text-sm"
                      >
